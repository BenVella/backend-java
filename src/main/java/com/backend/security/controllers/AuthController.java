...
import com.backend.security.models.ERole;
import com.backend.security.models.Role;
import com.backend.security.models.User;
...
public class AuthController {
    ...
    public ResponseEntity<?> authenticateUser(@RequestHeader(HttpHeaders.AUTHORIZATION) String authHeader) {
        ...
        var authToken = getBasicAuthToken(authHeader);
    ...
    private static UsernamePasswordAuthenticationToken getBasicAuthToken(String authHeader) {
        // Decode the Base64-encoded username:password from the Basic Auth header
        ...
        if (parts.length != 2) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid Basic Authorization format.");
        }

        String username = parts[0];
        String password = parts[1];

        // Now process the authentication as previously done
